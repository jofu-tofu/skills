---
name: ContextLayer
description: Generate and maintain slim, high-signal AGENTS.md context files for AI agents. USE WHEN the request involves creating, reviewing, correcting, shrinking, or refreshing AGENTS.md files — whether the user says so explicitly or describes symptoms like "agents are confused", "context is stale", "AGENTS.md is too big", "just started a new project", "agents keep putting things in the wrong place", "my skill is getting confused about its own structure", "agents don't know about our workaround", "why did we implement it this way is lost", "context hasn't been updated in a while", or "I just added a new module and agents don't know about it". Covers the full lifecycle of agent context files. For README, API docs, or non-AGENTS.md documentation, handle directly without this skill.
...
Generates and maintains the full hierarchical AGENTS.md tree for a project.
Minimizes **fusion friction** — every wrong or stale line in AGENTS.md causes
agents to operate with bad context and produce subtly wrong behavior.

**Context layer = tree**, not a single file. Root AGENTS.md for global
orientation; subdirectory AGENTS.md files for scoped domain context.
...
|----------|---------|------|
| **Generate** | No AGENTS.md exists, full rebuild needed, new project onboarding | `Workflows/Generate.md` |
| **Audit** | AGENTS.md may be wrong/outdated/incomplete, agent confusion reported | `Workflows/Audit.md` |
| **Prune** | AGENTS.md too large or noisy, token budget exceeded, reduce overhead | `Workflows/Prune.md` |
...
- **"look at my AGENTS.md" / bare "context layer"** → ask: *What's the problem you're trying to solve?*
  - No AGENTS.md yet → Generate
...
- **Generate** → parallel haiku agents per subsystem → synthesized AGENTS.md tree
...
- `PruningInstruction.md` — Template embedded at bottom of every generated AGENTS.md
...
```
// Explicit triggers
"generate a AGENTS.md for this project"          → Generate
...
"check my AGENTS.md for issues"                  → Audit
"is my AGENTS.md stale?"                         → Audit
"prune the context layer, it's getting bloated"  → Prune
"compress my AGENTS.md to save tokens"           → Prune

// Diagnostic phrasings (symptom → workflow)
"agents keep putting files in the wrong place"   → Audit (stale paths)
"Codex ignores my AGENTS.md, it's too long"     → Prune
...
"audit only the api subdirectory's AGENTS.md"    → Audit (targeted: src/api)
